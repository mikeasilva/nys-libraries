...
```{r trend_visulizations, results='asis', eval=FALSE, echo=FALSE}
visualize_trend <- function(df, measure){
  df <- df %>%
  group_by(year) %>%
  summarise(average = mean(measurement), observations = n())
  
  print(qplot(df$year, df$average, xlab='Year', ylab=paste('Average',measure)))
  
  #print(pandoc.table(df))
}

visualize_trend(visits, 'Visits')

visualize_trend(book_circulation,'Books Circulated')
```
